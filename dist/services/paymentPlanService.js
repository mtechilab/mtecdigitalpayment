 ...
 *  submission. */
export async function generateMtecReference(studentRowId) {
    const supabase = getSupabase();
 ...
 *  second time. */
export async function expireSubmission(submissionId) {
    const supabase = getSupabase();
    const { error } = await supabase
        ...
        .eq("status", "pending");
    if (error)
        throw new Error(`expireSubmission failed: ${error.message}`);
}
...
async function getNextRecurrentPaymentAmount(studentRowId) {
    const plan = await getActivePlanForStudent(studentRowId);
    const periods = await getPeriodsForPlan(plan.id);
    const unpaid = periods.filter((p) => p.status !== "paid");
    if (unpaid.length === 0)
        throw new Error("No unpaid payment period exists.");
    const next = unpaid[0];
 ...
 */
export async function ensureRecurrentPaymentCode(studentRowId) {
    const supabase = getSupabase();
    const plan = await getActivePlanForStudent(studentRowId);
    ...
    if (plan.monime_recurrent_code_id && plan.monime_recurrent_ussd_code && plan.monime_recurrent_expire_time) {
        const expiry = new Date(plan.monime_recurrent_expire_time);
        if (expiry.getTime() > Date.now()) {
            return {
                codeId: plan.monime_recurrent_code_id,
                ussdCode: plan.monime_recurrent_ussd_code,
                expireTime: plan.monime_recurrent_expire_time,
    ...
    const { error: updateError } = await supabase
        ...
        .eq("id", plan.id);
    if (updateError)
        throw new Error(`Failed to save recurrent payment code: ${updateError.message}`);
    return {
