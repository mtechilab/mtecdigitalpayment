...
router.post("/webhook", express.raw({ type: "application/json" }), async (req, res) => {
    ...
    if (eventName === "payment_code.expired") {
        ...
        if (oneTimeSubmissionId) {
            try {
                ...
                console.error("[webhook] expireSubmission failed:", err.message);
            }
        }
    ...
    if (paymentCodeId) {
        ...
        if (plan) {
            ...
            const mtecReference = await generateMtecReference(plan.student_row_id);
            ...
            try {
                ...
                console.error("[webhook] recurrent finalize failed:", err.message);
