...
import { getThread, sendMessage } from "../services/chatService.js";
...
// GET /student/chat — this student's own thread with staff
router.get("/chat", async (req: AuthenticatedRequest, res: Response) => {
  try {
    const messages = await getThread(req.studentRowId!);
    res.json(messages);
  } catch (err) {
    console.error("[/student/chat] error:", (err as Error).message);
...
router.post("/chat", async (req: AuthenticatedRequest, res: Response) => {
  try {
    ...
    const { data: student } = await supabase
      .from("students").select("full_name").eq("id", req.studentRowId!).maybeSingle();

    await sendMessage(req.studentRowId!, "student", student?.full_name || "Student", message.trim());
    res.json({ success: true });
  } catch (err) {
