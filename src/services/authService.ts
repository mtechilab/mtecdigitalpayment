...
/** For a student changing their password later (not first login) — requires
 *  the current password to match first, same security bar as any normal
 *  "change password" flow. */
export async function changePassword(studentRowId: string, currentPassword: string, newPassword: string):
  Promise<{ outcome: "success" } | { outcome: "incorrect_current_password" } | { outcome: "not_found" }> {
  const supabase = getSupabase();
  const { data: student, error } = await supabase
    .from("students").select("password_hash").eq("id", studentRowId).maybeSingle();
  if (error || !student) return { outcome: "not_found" };
  ...
  const passwordHash = await bcrypt.hash(newPassword, 10);
  await supabase.from("students").update({ password_hash: passwordHash }).eq("id", studentRowId);
