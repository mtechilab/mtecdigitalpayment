...
/** Every manual-method submission awaiting review — cash, Orange Money
 *  (manual/agent), and alt. account, all of it, not just cash. This is
 *  what the admin-authenticated Payment Verification screen uses; the
 *  older /staff stopgap above only ever covered cash. Monime is excluded
 *  on purpose — those are confirmed automatically by the webhook and
 *  should never need a human to "verify" them here. */
/** Manual-method submissions awaiting review, PLUS Monime submissions
 *  stuck in "pending" — a fallback for when the webhook fails to confirm
 *  a payment that genuinely went through on Monime's side. Monime is
 *  still meant to be automatic; this is a safety net for staff to use
 *  when it isn't, not the normal path. Staff should double-check with
 *  the student/Monime dashboard before manually verifying one of these,
 *  since — unlike cash — there's no physical handoff moment confirming
 *  the money actually moved. */
export async function getPendingSubmissions() {
  const supabase = getSupabase();
  const { data, error } = await supabase
    .from("payment_submissions")
    .select("id, mtec_reference, amount, method, provider_reference, status, created_at, students(student_id, full_name)")
    .or("and(method.neq.monime,status.eq.under_review),and(method.eq.monime,status.eq.pending)")
    .order("created_at", { ascending: true });
  if (error) throw new Error(`getPendingSubmissions failed: ${error.message}`);
  return data;
}
